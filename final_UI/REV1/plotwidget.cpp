...
const std::vector<float> PlotWidget::correctionValues = {
...
PlotSettings globalPlotSettings;
...
//    }
    ...
    if (cursor1Enabled) {
        float x = leftMargin + cursor1Position * w;
        painter.drawLine(QPointF(x, 0), QPointF(x, height() - bottomMargin));
    }
    if (cursor2Enabled) {
        float x = leftMargin + cursor2Position * w;
        painter.drawLine(QPointF(x, 0), QPointF(x, height() - bottomMargin));
    }
...
void PlotWidget::setCursor1Position(float position)
{
    cursor1Position = std::max(0.0f, std::min(position, 1.0f));
    update(); // Trigger repaint
}

void PlotWidget::setCursor2Position(float position)
{
    cursor2Position = std::max(0.0f, std::min(position, 1.0f));
    update(); // Trigger repaint
}

void PlotWidget::setCursor1Enabled(bool enabled)
{
    cursor1Enabled = enabled;
    update();
}

void PlotWidget::setCursor2Enabled(bool enabled)
{
    cursor2Enabled = enabled;
    update();
}
