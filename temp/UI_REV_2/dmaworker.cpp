...
// -------- REAL HARDWARE MODE --------
    ...
    for (int i = NOISE_START; i<NOISE_END; i++){
        ...
        noiseSum += 10.0f * std::log10(val + 1e-12f);
        //noiseSum += val;
    }
    float noiseFloor = noiseSum / 200;
    ...
    for (int i = NOISE_START; i < NOISE_END; i++) {
        ...
        float diff = val - noiseFloor;
        varianceSum += diff * diff;
    }

    float variance = varianceSum / (NOISE_END - NOISE_START);
    float noiseSpread = std::sqrt(variance);

    emit newFFTData(noiseSpread, noiseFloor, freq_index, fft);
