...
float calculateSpread(const std::vector<float>& arr, int offset, int size)
{
    if (size <= 0) return 0.0f;

    float minVal = arr[offset];
    float maxVal = arr[offset];
...
bool isValidSinglePeak(const std::vector<float>& arr, int offset, int size, float threshold)
{
    int count = 0;
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        if (v >= threshold)
        {
            ...
            // early exit if more than one peak
            if (count > 1)
                return false;
        }
    }
