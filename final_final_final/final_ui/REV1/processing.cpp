...
// -------- Vertical Cursor → Frequency ----------
float PlotWidget::getVCursor1Freq() const
{
    return visibleStartFreq +
           (vCursor1Pos / 1000.0f) * (visibleEndFreq - visibleStartFreq);
}

float PlotWidget::getVCursor2Freq() const
{
    return visibleStartFreq +
           (vCursor2Pos / 1000.0f) * (visibleEndFreq - visibleStartFreq);
}

// -------- Horizontal Cursor → Amplitude ----------
float PlotWidget::getHCursor1Value() const
{
    return minVal + (1.0f - hCursor1Pos / 1000.0f) * (maxVal - minVal);
}

float PlotWidget::getHCursor2Value() const
{
    return minVal + (1.0f - hCursor2Pos / 1000.0f) * (maxVal - minVal);
}
