    ...
    ui(new Ui::SettingsDialog)
{
    ui->setupUi(this);
      ...
      ui->frequencyLabel->setText(QString::number(frequency_offset_1, 'f', 0) + "Hz");
      ui->stepsizeLabel->setText(QString::number(step_size_1, 'f', 0) + "Hz");
      ...
      connect(ui->frequencyButton, &QPushButton::clicked, this, [this]() {
          NumPadDialog dialog(this);
          dialog.setTitle("Ente Frequency Offset");
          if(dialog.exec() == QDialog::Accepted)
          {
              frequency_offset = dialog.value();
              ui->frequencyLabel->setText(QString::number(frequency_offset, 'f', 0) + " Hz");
          }
      });

      connect(ui->StepSizeButton, &QPushButton::clicked, this, [this]() {
          NumPadDialog dialog(this);
          dialog.setTitle("Ente Frequency Offset");
          if(dialog.exec() == QDialog::Accepted)
          {
              step_size = dialog.value();
              ui->stepsizeLabel->setText(QString::number(step_size, 'f', 0) + " Hz");
          }
      });
...
int SettingsDialog::frequencyOffset() const {
    return frequency_offset;
}

int SettingsDialog::stepSize() const {
    return step_size;
}
